...
export default {
  ...
  getDoctorAllDoctorEndpoint:
    "/api/auth/doctors/byDistance?lat={lattitude}&lng={longittude}",
  getDoctorWithDistanceEndpoint:
    "/api/auth/doctors/byDistance?lat={lattitude}&lng={longittude}&distance={distance}",
  getAppointmentPriscriptionEndpoint:
    "/doctor/prescription/by-appointment?appointmentId={AppointmentId}",
  AddPriscriptionEndpoint: "/doctor/prescription",
  getMedicalHistoryEndpoint: "/doctor/prescription?petId={PetUid}",
  /*
   * Pet Endpoint
   */
  ...
  getBookedAppoinmentEndpoint:
    "api/appointments/my-appointments-doctor?status=BOOKED",
  bookOfflineAppointment: "/api/appointments/book-offline-simple",
  getSlotByDoctorEndpoint:
    "/api/appointments/getSlot?doctorId={doctorId}&doctorDayId={doctorDayId}&date={date}",
  ...
  getAllGroomerByDistanceEndpoint:
    "/groomerkyc/nearby?latitude={latitude}&longitude={longitude}&maxDistance={Distance}&page={page}&size=10",

  behaviouristToClientKyc: "/api/behaviorist-kyc",
  getStatusBehaviouristToClient: "/api/behaviorist-kyc/get-status",
  getAllBehaviouristByDistance:
    "/behaviouristkyc/nearby?latitude={latitude}&longitude={longitude}&maxDistance={Distance}&page={page}&size=10",

  //stripe verify payment
  // verifyAppointmentPaymentEndpoint: '/api/appointments/verify-payment/{SessionId}',
  verifyAppointmentPaymentEndpoint:
    "{UserTypeEndpoint}/verify-payment/{SessionId}",
  ...
  getAvailableWalkerslotEndPoint:
    "/api/walker-appointments/available-slots?petWalkerUid={walkerUid}&petWalkerDayUid={dayUid}&date={date}",
  //behaviourist-client side
  getAvailableBehaviouristDay: "/api/behaviourist-days/service-provider/{Uid}",
  getAvailableSlotBehaviourist:
    "/api/behaviourist-appointments/available-slots?serviceProviderUid={providerUid}&behaviouristDayUid={dayUid}&date={date}",
  ...
  getGroomerBookedAppointmnetforClient:
    "/groomer/appointment/user/appointments",
