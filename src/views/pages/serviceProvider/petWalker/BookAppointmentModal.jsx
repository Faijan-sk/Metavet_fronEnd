...
function BookWalkModal({ walker, onClose }) {
  ...
  const handlePetSelectChange = (e) => {
    ...
    if (val === "__add_new__") {
      ...
      // Reset downstream when pet changes
      setSelectedDate(null);
      setSelectedSlot(null);
      setWalkerDayUid(null);
      setAvailableSlots([]);
      ...
      >
        ...
        <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl max-h-[92vh] overflow-y-auto">
          ...
          <form onSubmit={handleBookWalk} className="px-6 py-5 space-y-5">
            {/* STEP 1 — PET SELECT (always visible) */}
            <div className={!selectedPet ? "min-h-[200px]" : ""}>
              ...
              {/* Hint when no pet selected */}
              {!selectedPet && (
                <div className="mt-4 p-4 bg-[#52B2AD]/5 rounded-xl border border-[#52B2AD]/10">
            ...
            {/* STEP 2 & 3 — only show when pet is selected */}
            {selectedPet && (
              <div className="space-y-5">
                ...
                {/* STEP 3 — TIME SLOTS (shows after date selected) */}
                {selectedDate && (
                  <div>
