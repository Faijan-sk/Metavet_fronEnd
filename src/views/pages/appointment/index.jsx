...
const Appointment = () => {
  ...
  const appointmentStats = useMemo(() => {
    ...
    const completed = appointments.filter((a) => getActualStatus(a) === "completed").length;
    const cancelled = appointments.filter((a) => getActualStatus(a) === "cancelled").length;
      ...
      try {
        ...
        const transformedAppointments = rawAppointments.map((apt) => {
          ...
          return {
            ...
            time: apt.slotStartTime && apt.slotEndTime
  ...
  useEffect(() => {
    ...
    if (modalOpen || deleteConfirmModal) window.addEventListener("keydown", onKey);
  ...
  const EmptyState = () => (
    <div className="flex flex-col items-center justify-center py-20 px-4">
      ...
      <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6 max-w-3xl">
        <div className="text-center p-4"><div className="text-4xl mb-2">💉</div><p className="text-gray-700 font-medium">Vaccinations</p></div>
        <div className="text-center p-4"><div className="text-4xl mb-2">🏥</div><p className="text-gray-700 font-medium">Check-ups</p></div>
        <div className="text-center p-4"><div className="text-4xl mb-2">🦷</div><p className="text-gray-700 font-medium">Dental Care</p></div>
      </div>
    </div>
  );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center" aria-modal="true" role="dialog">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
        <div className="p-6">
          ...
          <h2 className="text-2xl font-bold text-center text-gray-800 mb-2">Cancel Appointment?</h2>
          <p className="text-center text-gray-600 mb-6">Are you sure you want to cancel this appointment? This action cannot be undone.</p>
          <div className="bg-gray-50 rounded-lg p-4 mb-6 space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-gray-600">Appointment ID:</span><span className="font-semibold">#{appointment.id}</span></div>
            <div className="flex justify-between"><span className="text-gray-600">Pet:</span><span className="font-semibold">{appointment.petName} ({appointment.petSpecies})</span></div>
            <div className="flex justify-between"><span className="text-gray-600">Doctor:</span><span className="font-semibold">{appointment.doctorName}</span></div>
            <div className="flex justify-between"><span className="text-gray-600">Specialization:</span><span className="font-semibold">{appointment.doctorSpecialization}</span></div>
            <div className="flex justify-between"><span className="text-gray-600">Date:</span><span className="font-semibold">{appointment.date}</span></div>
            <div className="flex justify-between"><span className="text-gray-600">Time:</span><span className="font-semibold">{appointment.time}</span></div>
            <div className="flex justify-between"><span className="text-gray-600">Hospital:</span><span className="font-semibold">{appointment.hospitalName}</span></div>
            <div className="flex justify-between"><span className="text-gray-600">Fees:</span><span className="font-semibold text-[#52B2AD]">₹{appointment.appointmentFees}</span></div>
          </div>
          <div className="flex gap-3">
            <button onClick={onCancel} disabled={isDeleting} className="flex-1 px-4 py-3 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded-lg font-semibold transition-colors disabled:opacity-50">Keep Appointment</button>
            <button onClick={onConfirm} disabled={isDeleting} className="flex-1 px-4 py-3 bg-red-600 hover:bg-red-700 text-white rounded-lg font-semibold transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
              {isDeleting ? (<><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" />Cancelling...</>) : (<><AlertTriangle size={18} />Yes, Cancel</>)}
            </button>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 py-8 px-4">
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            {userInfo ? (
              <div className="flex gap-4">
                {userInfo.userType === 2 && (
                  <button
                    onClick={() => { setModalType("doctorBook"); setModalOpen(true); }}
              ...
              <button onClick={() => (window.location.href = "/Signin")} className="bg-gray-400 text-white px-6 py-3 rounded-full shadow-lg flex items-center gap-2 font-semibold cursor-pointer">
      ...
      {modalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" aria-modal="true" role="dialog">
          <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={() => { setModalOpen(false); setModalType(null); }} />
          <div className="relative w-full max-w-3xl mx-auto">
            {modalType === "book" && <BookAppointmentForm onClose={() => { setModalOpen(false); setModalType(null); }} onCreated={(a) => { if (a?.id) setAppointments((p) => [a, ...p]); setModalOpen(false); setModalType(null); }} />}
            {modalType === "doctorBook" && <CreateDoctorAppointment onClose={() => { setModalOpen(false); setModalType(null); }} onCreated={(a) => { if (a?.id) setAppointments((p) => [a, ...p]); setModalOpen(false); setModalType(null); }} />}
            {modalType === "create" && <CreateAppointment onClose={() => { setModalOpen(false); setModalType(null); }} onCreated={(a) => { if (a?.id) setAppointments((p) => [a, ...p]); setModalOpen(false); setModalType(null); }} />}
          </div>
        </div>
      )}
