...
const AppointmentListing = ({ appointments, onUpdate, onDelete }) => {
  ...
  const handleSaveEdit = () => { if (onUpdate) onUpdate(editAppointment); setEditAppointment(null); };
    ...
    <div>
      ...
      <div className="space-y-6 mb-8">
        {currentAppointments.map((appointment, index) => {
          ...
          return (
            ...
            >
              {/* Colored Side Bar */}
              <div className={`h-2 ${actualStatus === 'completed' ? 'bg-[#42948f]' : actualStatus === 'cancelled' ? 'bg-[#52B2AD]/40' : 'bg-[#52B2AD]'}`} />
              ...
              <div className="flex items-center justify-between p-6 pb-4 border-b border-gray-100">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <span className={`inline-block px-3 py-1 rounded-full text-xs font-semibold mt-1 ${getStatusColor(actualStatus)}`}>
                ...
                <div className="flex gap-2">
                  {editAppointment?.id === appointment.id ? (
                    <>
                      <button onClick={handleSaveEdit} className="p-2.5 rounded-full bg-[#52B2AD]/20 text-[#52B2AD] hover:bg-[#52B2AD]/30 transition-all transform hover:scale-110" title="Save">
                        <Save size={20} />
                      </button>
                      <button onClick={handleCancelEdit} className="p-2.5 rounded-full bg-gray-100 text-gray-600 hover:bg-gray-200 transition-all transform hover:scale-110" title="Cancel">
                        <X size={20} />
                      </button>
                    </>
                  ) : (
                  ...
                  <div className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input type="text" name="petName" value={editAppointment.petName} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#52B2AD] focus:border-transparent" />
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-1">Doctor</label>
                        <input type="text" name="doctorName" value={editAppointment.doctorName} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#52B2AD] focus:border-transparent" />
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-1">Date</label>
                        <input type="date" name="date" value={editAppointment.date} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#52B2AD] focus:border-transparent" />
                      </div>
                      <div>
                        <label className="block text-sm font-semibold text-gray-700 mb-1">Time</label>
                        <input type="text" name="time" value={editAppointment.time} onChange={handleInputChange} className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#52B2AD] focus:border-transparent" />
                      </div>
                    </div>
                  </div>
                ) : (
                  ...
                  <div className="flex items-start gap-6">
                    ...
                    <div className="flex-shrink-0">
                      <div className="relative">
                        <img src={dr} alt={appointment.doctorName} className="w-20 h-20 rounded-full border-4 border-[#52B2AD] shadow-lg object-cover bg-white" />
                    ...
                    <div className="flex-1 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                      ...
                      <div>
                        ...
                        {appointment.doctorQualification && (
                          <p className="text-xs text-gray-400">{appointment.doctorQualification}</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        {appointment.petSpecies && (
                          <p className="text-xs text-gray-500">{appointment.petSpecies}</p>
                        )}
                        {appointment.petBreed && (
                          <p className="text-xs text-gray-400">{appointment.petBreed}</p>
                        )}
                        {appointment.petAge != null && (
                          <p className="text-xs text-gray-400">{appointment.petAge} yrs</p>
                        )}
                      </div>
                      ...
                      <div>
                        ...
                        <p className="font-bold text-[#52B2AD]">₹{appointment.appointmentFees ?? 0}</p>
                      </div>
      ...
      {appointments.length > itemsPerPage && (
        <div className="flex items-center justify-center gap-6 mt-8">
          <button onClick={handlePrevious} className="p-3 rounded-full bg-[#52B2AD] text-white hover:bg-[#42948f] transition-all transform hover:scale-110 shadow-lg" title="Previous">
            <ChevronLeft size={24} />
          </button>
          ...
          <button onClick={handleNext} className="p-3 rounded-full bg-[#52B2AD] text-white hover:bg-[#42948f] transition-all transform hover:scale-110 shadow-lg" title="Next">
