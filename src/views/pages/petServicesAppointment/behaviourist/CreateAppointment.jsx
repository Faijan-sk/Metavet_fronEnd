...
const BookAppointmentForm = ({ onClose, onCreated, initialValues = null }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await useJwt.createBehaviouristSlotAndDay(payload);
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
        ...
        <div className="flex flex-col md:flex-row gap-3">
          <div className="relative flex-1">
            ...
            {dayDropdownOpen && (
              <ul className="absolute z-50 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-xl max-h-48 overflow-y-auto">
                {DAYS.map((d) => (
                  <li key={d} onClick={() => { setSelectedDay(d); setDayDropdownOpen(false); }} className="px-4 py-2 hover:bg-[#52B2AD] hover:text-white cursor-pointer transition-colors">
                    {d}
                  </li>
                ))}
              </ul>
          ...
          <button type="button" onClick={addDay} className="px-6 py-3 bg-[#52B2AD] text-white font-bold rounded-lg flex items-center justify-center gap-2 shadow-sm">
          ...
          <div key={i} className="flex flex-col md:flex-row gap-4 p-4 bg-white border border-gray-100 rounded-xl items-center shadow-sm">
            ...
            <div className="flex gap-4 w-full">
              <div className="flex-1">
                <input type="time" value={d.startTime} onChange={(e) => updateDayTime(i, { startTime: e.target.value })} className="w-full border rounded-lg p-2 outline-[#52B2AD]" required />
              </div>
              <div className="flex-1">
                <input type="time" value={d.endTime} onChange={(e) => updateDayTime(i, { endTime: e.target.value })} className="w-full border rounded-lg p-2 outline-[#52B2AD]" required />
              </div>
            </div>
        ...
        <div className="grid grid-cols-3 md:grid-cols-5 gap-2">
          {slotOptions.map(opt => (
            <button
              key={opt}
              type="button"
              onClick={() => setSlotType(opt)}
              className={`py-2 px-2 rounded-lg border-2 text-sm font-semibold transition-all ${
                slotType === opt ? "border-[#52B2AD] bg-[#52B2AD]/10 text-[#52B2AD]" : "border-gray-100 text-gray-400 hover:border-gray-200"
              }`}
            >
      ...
      <div className="flex justify-end gap-3 pt-6 border-t mt-6">
        <button type="button" onClick={onClose} className="px-6 py-2.5 text-gray-500 font-bold hover:bg-gray-100 rounded-lg transition-all">Cancel</button>
        <button type="submit" disabled={loading} className="px-10 py-2.5 bg-gradient-to-r from-[#52B2AD] to-[#42948f] text-white rounded-lg font-bold shadow-md hover:shadow-lg transition-all">
